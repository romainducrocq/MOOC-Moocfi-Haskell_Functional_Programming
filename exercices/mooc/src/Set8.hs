...
combine f (Picture g) (Picture h) = Picture (\(Coord x y) -> f (g (Coord x y)) (h (Coord x y)))
...
union shp1 shp2 = Shape (\(Coord x y) -> contains shp1 x y || contains shp2 x y)

cut :: Shape -> Shape -> Shape
cut shp1 shp2 = Shape (\(Coord x y) -> contains shp1 x y && not (contains shp2 x y))
...
paintSolid col shp (Picture g) = Picture f
    where f (Coord x y) | contains shp x y = col
                        | otherwise = g (Coord x y)
...
paint (Picture g) shp (Picture h) = Picture f
    where f (Coord x y) | contains shp x y = g (Coord x y)
                        | otherwise = h (Coord x y)
...
blurCoords :: Coord -> [Coord]
blurCoords (Coord x y) = map (uncurry Coord) [(x,y),(x+1,y),(x-1,y),(x,y+1), (x,y-1)]

blur :: Picture -> Picture
blur (Picture g) = Picture (f . blurCoords)
    where f = (\(Color r g b) -> Color (div r 5) (div g 5) (div b 5))
              . foldl1 (\(Color r1 g1 b1) (Color r2 g2 b2) -> Color (r1+r2) (g1+g2) (b1+b2))
              . map g
...
blurMany :: Int -> Picture -> Picture
blurMany 0 img = img
blurMany i img = blurMany (i-1) (blur img)

instance Transform BlurMany where
  apply (BlurMany n) = blurMany n

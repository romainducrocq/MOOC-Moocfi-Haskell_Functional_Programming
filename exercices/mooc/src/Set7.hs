...
member m (Set ms) = m `elem` ms

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add x (Set xs)
    | Set xs == emptySet = Set [x]
    | member x (Set xs)  = Set xs
    | otherwise          = Set $ go 0 xs
    where
      go _ [] = xs ++ [x]
      go i (y:ys)
          | x < y = take i xs ++ (x:y:ys)
          | otherwise = go (i+1) ys
...
step Error    _        = Error
step Start    AddEggs  = S1
step S1       AddFlour = S2
step S1       AddSugar = S3
step S2       AddSugar = S4
step S3       AddFlour = S4
step S4       Mix      = S5
step S5       Bake     = Finished
step _        _        = Error
...
reverseNonEmpty (x :| xs) = (\ys -> head ys :| tail ys) . reverse $ (x:xs)
...
instance Ord a => Semigroup (Set a) where
    set <> Set xs = go xs set
        where
          go [] s     = s
          go (y:ys) s = go ys $ add y s
...
passwordAllowed pw (And pwrq1 pwrq2)       = passwordAllowed pw pwrq1 && passwordAllowed pw pwrq2
passwordAllowed pw (Or pwrq1 pwrq2)        = passwordAllowed pw pwrq1 || passwordAllowed pw pwrq2

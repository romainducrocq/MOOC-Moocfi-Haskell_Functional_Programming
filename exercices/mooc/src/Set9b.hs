...
{-
prettyPrint s xs = go s s
    where
      go 0 _ = []
      go i 0 = '\n' : go (i-1) s
      go i j
          | (s-i+1,s-j+1) `elem` xs = 'Q' : go i (j-1)
          | otherwise               = '.' : go i (j-1)
-}
prettyPrint s xs = go s s $ sort xs
    where
      go 0 _ _  = []
      go i 0 ys = '\n' : go (i-1) s ys
      go i j [] = '.' : go i (j-1) []
      go i j (y:ys)
          | (s-i+1,s-j+1) == y = 'Q' : go i (j-1) ys
          | otherwise          = '.' : go i (j-1) (y:ys)
...
sameDiag (i,j) (k,l) = abs (i-k) == abs (j-l) && compare i k == compare j l

sameAntidiag :: Coord -> Coord -> Bool
sameAntidiag (i,j) (k,l) = (abs (i-k) == abs (j-l) && compare i k /= compare j l) || (i,j) == (k,l)
...
danger _ [] = False
danger y (x:xs)
    | or [sameRow y x, sameCol y x, sameDiag y x, sameAntidiag y x] = True
    | otherwise                                                     = danger y xs
...
prettyPrint2 s xs = go s s $ sort xs
    where
      dg i j = if danger (s-i+1,s-j+1) xs then '#' else '.'
      go 0 _ _  = []
      go i 0 ys = '\n' : go (i-1) s ys
      go i j [] = dg i j : go i (j-1) []
      go i j (y:ys)
          | (s-i+1,s-j+1) == y = 'Q' : go i (j-1) ys
          | otherwise          = dg i j : go i (j-1) (y:ys)
...
fixFirst s ((i,j):xs)
    | j > s           = Nothing
    | danger (i,j) xs = fixFirst s ((i,j+1):xs)
...
step s xs = go $ fixFirst s xs
    where
      go fx
          | isJust fx = continue $ fromJust fx
          | otherwise = backtrack xs
...
finish s (x:xs)
    | length xs == s = xs
    | otherwise      = finish s $ step s (x:xs)

...
deposit db t x =
    let q = (t,x) :: (T.Text,Int)
    in  execute db depositQuery q
...
balance db t =
    let q = [t] :: [T.Text]
    in
        (query db balanceQuery q :: IO [[Int]])
        <&>
        foldr (\r acc -> head r + acc) 0
...
data Command = Balance T.Text | Deposit T.Text Int | Withdraw T.Text Int
  deriving (Show, Eq)
...
parseCommand ["balance",t] = Just . Balance $ t
parseCommand [c,t,x]
    | c == "deposit"       = if isJust m then Just . Deposit t . fromJust $ m else Nothing
    | c == "withdraw"      = if isJust m then Just . Withdraw t . fromJust $ m else Nothing
    | otherwise            = Nothing
    where m = parseInt x
...
perform db (Just (Balance t))    =
    balance db t
    <&>
    T.pack . show
perform db (Just (Deposit t x))  =
    deposit db t x
    >>
    (return . T.pack $ "OK")
perform db (Just (Withdraw t x)) =
    deposit db t (-x)
    >>
    (return . T.pack $ "OK")
perform _  Nothing               =
...
server db request respond = do
     perform db . parseCommand . pathInfo $ request
     >>=
     (respond . responseLBS status200 [] . encodeResponse)

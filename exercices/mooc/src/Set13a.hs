...
checkNumber (xs,ys) =
    if   any (any Data.Char.isDigit) [xs,ys]
    then Nothing
    else Just (xs,ys)
...
checkCapitals (x:xs,y:ys) =
    if   all Data.Char.isUpper [x,y]
    then Just (x:xs,y:ys)
    else Nothing
...
winner xs p1 p2 =
    mapM (`lookup` xs) [p1, p2]
    ?>
    (Just . fst . head . sortOn (negate . snd) . zip [p1,p2])
...
balance accountName = BankOp $ \(Bank b) ->
    (Map.findWithDefault 0 accountName b, Bank b)

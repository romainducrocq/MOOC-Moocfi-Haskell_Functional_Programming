...
shout :: T.Text -> T.Text
shout = T.unwords . zipWith (\i w -> (if even i then T.toUpper w else w)) [0..] . T.words
...
longestRepeat =  foldr (max . T.length) 0 . T.group
...
takeStrict x t = TL.toStrict $ TL.take x t
...
xorChecksum = foldr xor 0 . B.unpack
...
countUtf8Chars = (\t ->
    if   isLeft t
    then Nothing
    else Just . T.length . fromRight undefined $ t
    ) . decodeUtf8'
...
pingpong b = BL.pack . cycle . B.unpack $ b <> B.reverse b

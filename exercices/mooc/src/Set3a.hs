...
whileRight check x
    | isLeft (check x) = fromLeft undefined (check x)
    | otherwise = whileRight check (fromRight undefined (check x))
...
joinToLength i strs = [xs | fst <- strs, snd <- strs, let xs = fst ++ snd, length xs == i]
...
multiApp f gs x = f $ fs gs x
    where
      fs [] _ = []
      fs (g:gs) x = g x : fs gs x
...
interpreter commands = go commands [[0,0],[]]
  where
    ...
    go [] (xy:prints:_) = reverse . map show $ prints
    go (cmd:cmds) (xy:prints:_) = go cmds (case cmd of "up"     -> [zipWith (+) [0,  1] xy, prints]
                                                       "down"   -> [zipWith (+) [0, -1] xy, prints]
                                                       "left"   -> [zipWith (+) [-1, 0] xy, prints]
                                                       "right"  -> [zipWith (+) [1,  0] xy, prints]
                                                       "printX" -> [xy, head xy : prints]
                                                       "printY" -> [xy, last xy : prints])

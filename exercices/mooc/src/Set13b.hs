...
visit m x =
    ...
    \xs ->
        unless (x `elem` xs) $
            modify (x:)
            >>
            mapM_ (visit m) (fromMaybe [] $ x `lookup` m)
...
path m p1 p2 = p2 `elem` execState (visit m p1) []
...
-}
findSum2 ks ns = [(x,y,x+y) | x <- ks, y <- ks, (x+y) `elem` ns]
...
allSums :: [Int] -> [Int]
allSums xs = [sum $ zipWith (*) xs $
        map (\c -> fromEnum c - fromEnum '0') $ showIntAtBase 2 intToDigit i ""
    | i <- [0..2^length xs-1]]
...
newtype SL a = SL (Int -> (a,Int,[String]))
...
instance Monad SL where
  ...
  return x = SL (x,,[])
  op >>= f = SL $ \s ->
      (\(x,s',la) -> (\(y,s'',lb) -> (y,s'',la++lb)) $ runSL (f x) s') $ runSL op s

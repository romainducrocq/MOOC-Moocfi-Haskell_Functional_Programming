...
-- validateDiv x y = liftA2 div (pure x) (check (y/=0) "Division by zero!" y)
validateDiv x y =
    div x <$> check (y/=0) "Division by zero!" y
...
validateAddress streetName streetNumber postCode =
    check (length streetName <= 20)
    "Invalid street name"
    (Address streetName streetNumber postCode)
    *>
    check (all isDigit streetNumber)
    "Invalid street number"
    (Address streetName streetNumber postCode)
    *>
    check (all isDigit postCode &&
        length postCode == 5)
    "Invalid postcode"
    (Address streetName streetNumber postCode)
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
    (++)
    <$>
    ((\x y z -> [Person x y z]) <$> name1 <*> age1 <*> employed1)
    <*>
    ((\x y z -> [Person x y z]) <$> name2 <*> age2 <*> employed2)
...
boolOrInt s =
    check (isJust (readMaybe s :: Maybe Bool))
    "Not a Bool"
    (Left (read s :: Bool))
    <|>
    check (isJust (readMaybe s :: Maybe Int))
    "Not an Int"
    (Right (read s :: Int))
...
parseExpression s =
    if  length (words s) == 3
    then
        let [s1,op,s2] = words s
            b00 = op == "+"
            b01 = all isDigit s1
            b02 = all isDigit s2
            b10 = b00 || op == "-"
            b11 = b01 || (length s1 == 1 && (isAlpha . head) s1)
            b12 = b02 || (length s2 == 1 && (isAlpha . head) s2)
        in
            if b10 && b11 && b12
            then
                let arg1 = if b01 then Number (read s1 :: Int) else Variable . head $ s1
                    arg2 = if b02 then Number (read s2 :: Int) else Variable . head $ s2
                in
                    pure $ if b00 then Plus arg1 arg2 else Minus arg1 arg2
            else
                foldr (\(b,msg) -> (*>) $ check b msg undefined) empty [
                    (b10,"Unknown operator: " ++ op),
                    (b11,"Invalid number: "   ++ s1),
                    (b11,"Invalid variable: " ++ s1),
                    (b12,"Invalid number: "   ++ s2),
                    (b12,"Invalid variable: " ++ s2)]
    else
        check False ("Invalid expression: " ++ s) undefined
...
instance Functor Priced where
  fmap f (Priced x v) = Priced x $ f v

instance Applicative Priced where
  pure                                   = Priced 0
  liftA2 f (Priced x1 v1) (Priced x2 v2) = Priced (x1+x2) (f v1 v2)

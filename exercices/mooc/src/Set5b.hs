...
treeMax Empty = 0
treeMax (Node x t1 t2) =  max (max x $ treeMax t1) (max x $ treeMax t2)
...
allValues cond (Node x t1 t2)
    | cond x = all ((==True) . allValues cond) [t1,t2]
    | otherwise = False
...
cull _ Empty = Empty
cull val (Node x t1 t2)
     | x == val = Empty
     | otherwise = Node x (cull val t1) (cull val t2)
...
isOrdered (Node x t1 t2) = allValues (<x) t1 &&
                           allValues (>x) t2 &&
                           isOrdered t1      &&
                           isOrdered t2
...
walk (step:steps) (Node _ t1 t2) = walk steps $
                                   if step == StepL then t1 else t2
...
set _ _ Empty = Empty
set [] val (Node _ t1 t2) = Node val t1 t2
set (step:steps) val (Node x t1 t2)
    | step == StepL = Node x (set steps val t1) t2
    | otherwise     = Node x t1 (set steps val t2)
...
search val t
    | allValues (/=val) t = Nothing
    | otherwise = Just $ go t
    where
      go (Node x t1 t2)
          | val == x = []
          | allValues (/=val) t2 = StepL : go t1
          | otherwise            = StepR : go t2
-}
search val t = sequence $ go t
    where
      go Empty = [Nothing]
      go (Node x t1 t2)
          | val == x = []
          | allValues (/=val) t2 = Just StepL : go t1
          | otherwise            = Just StepR : go t2

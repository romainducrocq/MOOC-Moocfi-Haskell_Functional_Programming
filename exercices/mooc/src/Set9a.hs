...
countValid = length . filter (\str -> (str!!2 == str!!4) || (str!!3 == str!!5))
...
data Lock = MkLock { o :: Bool, c :: String } -- open, code
  deriving Show

-- aLock should be a locked lock with the code "1234"
aLock :: Lock
aLock = MkLock False "1234"
...
instance Eq Text where
    Text a == Text b = f a == f b
        where
          f = filter (\c -> c `notElem` [' ', '\n'])
...
-- compose xs ys = [(fst t, fromJust $ lookup (snd t) ys) | t <- xs, isJust $ lookup (snd t) ys]
compose xs ys = map (\t -> (fst t, fromJust $ lookup (snd t) ys))
    . filter (\t -> isJust $ lookup (snd t) ys)
    $ xs
...
permute p q = map snd . sortBy (\(x,_) (y,_) -> compare x y) $ zip p q

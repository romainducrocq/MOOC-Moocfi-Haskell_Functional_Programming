...
-- rangeOf xs = foldr (-) 0 (map ($ sort xs) [last, head])
rangeOf xs = (\ys -> last ys - head ys) $ sort xs
...
longest [xs] = xs
longest (xs:ys:xss)
    | length xs == length ys = longest $ minimumBy (\(a:as) (b:bs) -> compare a b) [xs,ys] : xss
    | otherwise = longest $ maximumBy (\as bs -> compare (length as) (length bs)) [xs,ys] : xss
...
--     | key == fst x = (fst x, snd x + 1) : incrementKey key xs
--     | otherwise = x : incrementKey key xs
incrementKey key = map (\x -> if key == fst x then (fst x, snd x + 1) else x)
...
winner scores player1 player2
    | Map.lookup player1 scores < Map.lookup player2 scores = player2
    | otherwise = player1
...
freqs xs = foldr (Map.alter (\(Just v) -> Just (v + 1))) (Map.fromList [(k,0) | k <- xs]) xs
...
transfer from to amount bank
    ...
    | amount < 0 = bank
    | fromJust (Map.lookup from bank) < amount = bank
    | otherwise = Map.insert to (fromJust (Map.lookup to bank) + amount) (Map.insert from (fromJust (Map.lookup from bank) - amount) bank)
    -- where
...
maxIndex arr = fst $ maximumBy (\a b -> compare (snd a) (snd b)) (Data.Array.assocs arr)

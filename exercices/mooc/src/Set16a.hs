...
sumIsLength input output =
    length input === foldr (\(_,x) y -> x+y) 0 output
...
inputInOutput input output =
    forAll (elements input) (\v -> elem v $ fst <$> output)
...
outputInInput input output =
    forAll (elements output) (\(v,x) -> x == length (filter (==v) input))
...
instance Arbitrary Arg where
  arbitrary = oneof [
      elements [0..10] <&> Number,
      elements ['a','b','c','x','y','z'] <&> Variable]

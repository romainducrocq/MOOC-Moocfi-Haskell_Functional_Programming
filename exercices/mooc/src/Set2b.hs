...
countdown n = "Ready! " ++ count n ++ "Liftoff!"
    where count :: Integer -> String
          count x
              | x == 0 = ""
              | otherwise = show x ++ "... " ++ count (x - 1)
...
smallestDivisor n  = mySd n 2
    where mySd :: Integer -> Integer -> Integer
          mySd x k
              | x `mod` k == 0 = k
              | otherwise = mySd x (k + 1)
...
biggestPrimeAtMost n
    | n < 2 = -1
    | otherwise = myBp n n
    where myBp :: Integer -> Integer -> Integer
          myBp x k
              | isPrime k = k
              | otherwise = myBp x (k - 1)